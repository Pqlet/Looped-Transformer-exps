...
from models_dilated import build_model
...
def train_step(args, curriculum, model, xs, ys, optimizer, ctx, scaler):
    if args.model.family in ['gpt2', 'gpt2_tying']:
        ...
        if ctx is not None:
            with ctx:
                # args.training.dilation_loop_window
                max_loops_for_grads = args.training.n_loop_window
                loops_calculate_grads = list(range(n_loops, 0-1, -args.training.dilation_loop_window))
                # leave only `max_loops_for_grads` last loops for gradient counting
                loops_calculate_grads = loops_calculate_grads[:max_loops_for_grads]
                y_pred_list = model(xs, ys, loops_calculate_grads, n_loops)
